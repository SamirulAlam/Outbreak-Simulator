...
class Simulation():
    ...
    def __init__(self):
        ...
        #Get simulation initial condition from the user
        ...
        #Convert the population size to nearest perfect square for visual purpose
        root=math.sqrt(self.population_size)
        #User didn't enter a perfect square for the population
        if int(root+.5)**2 != self.population_size:
            root=round(root,0)
            self.grid_size=int(root)
            self.population_size=self.grid_size**2
            print("Rounding population size to "+ str(self.population_size) + " for visual purpose")
        #User did enter a perfect square for the population
        else:
            self.grid_size=int(math.sqrt(self.population_size))

        print("\nWe must first start by infecting a portion of population.")
...
class Person():
    ...
    def update(self, simulation):
        """Update an individual person if the person is not dead. Check if they are infected If they are, increase the days infected count, then check if they should die or be healed."""
...
class Population():
    ...
    def initial_infection(self, simulation):
        """Infect an initial portion of the population based on initial condition of the simulation"""
        #Infect the infection percent*population size gives the total number to infect
        ...
        while infections <infected_count:
            # x is a random row in the population, y is a random person in the population row
            ...
            #If the person is not infected, infect themselves
    ...
    def spread_infection(self,simulation):
        ...
        for i in range(simulation.grid_size):
            #Loop through all the Person objects in a given row
            for j in range(simulation.grid_size):
                #Check to see if this given person self.population[i][j] is dead
                if self.population[i][j].is_dead == False:
                    #Check to se if we need to infect this person
                    #We will try infect the given person if an adjacent person is already infected
                    # If i==0 we are in the first row so we can not look above
                    if i==0:
                        #If j==0 we are in the first column so we can not look left
                        if j==0:
                            if self.population[i][j+1].is_infected or self.population[i+1][j].is_infected:
                                self.population[i][j].infect(simulation) 
                        #If we are in the last column we can not look right
                        ...
                        #If we are in any other column we can look left right or below
                            ...
                            if self.population[i][j-1].is_infected or self.population[i][j+1].is_infected or self.population[i+1][j].is_infected:
                                self.population[i][j].infect(simulation)
                    #If I==simulation.grid_size-1 we are in the last row so we cannot look below
                    ...
                    #Otherwise we are in a row in between we can look left right above or below
                        ...
                        #If we are in the last column we can not look right
                            ...
                            if self.population[i][j-1].is_infected or self.population[i+1][j].is_infected or self.population[i-1][j].is_infected:
                                self.population[i][j].infect(simulation)
                        #If we are in any other column we can look left right above or below
                        else:
    ...
    def update(self,simulation):
        ...
        #Loop through the population to access each row of
    ...
    def display_statistics(self,simulation):
        ...
        for row in self.population:
            #Loop through the row to access each person in
            for person in row:
                #Person is infected
...
def graphics(simulation,population,canvas):
    ...
    # Get the dimension of an individual square in a grid
    # Each square represents a person in the population
    # Use 600 for GUI window that is 600x600 
    ...
    for i in range(simulation.grid_size):
        ...
        #Loop through all person in teh rows
        for j in range(simulation.grid_size):
...
#Set constant varables for windows size
WINDOW_WIDTH =600
WINDOW_HEIGHT =600
